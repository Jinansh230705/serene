...
export default function ClientGallery({ 
  ...
  // Start with the shared index if provided
  const initialSharedIndex = sharedKey ? initialItems.findIndex(i => i.uniqueKey === sharedKey) : -1;
  const [selectedGridItemIndex, setSelectedGridItemIndex] = useState<number | null>(initialSharedIndex !== -1 ? initialSharedIndex : null);
  ...
  useEffect(() => {
    ...
    // Skip the very first fetch if it matches our initial server-rendered data
    if (isFirstRender.current) {
      isFirstRender.current = false;
      if (page === 1 && search === '' && category === 'All' && seed === initialSeed) {
        return;
      }
    }
  ...
  const handleRemoveLike = async (id: string, imageUrl?: string, uniqueKey?: string) => {
    try {
      const bodyPayload = imageUrl ? { id, imageUrl } : { id };
      ...
      if (response.ok) {
        ...
        setDeletedIds(prev => {
          ...
          if (uniqueKey) {
            next.add(uniqueKey); // Hide this specific grid item
          } else {
            // Hide all grid items related to this post
            gridItems.forEach(g => {
              if (g.id === id) next.add(g.uniqueKey);
            });
            next.add(id);
          }
          return next;
        ...
        if (selectedGridItemIndex !== null && gridItems[selectedGridItemIndex]?.id === id) {
          if (!uniqueKey || gridItems[selectedGridItemIndex].uniqueKey === uniqueKey) {
             setSelectedGridItemIndex(null);
          }
        }
      } else {
  ...
  const masonryColumns = useMemo(() => {
    ...
    gridItems.forEach((item) => {
      // Ignore if deleted by uniqueKey or entire id
      if (deletedIds.has(item.uniqueKey) || deletedIds.has(item.id)) return;
    ...
    <div className="min-h-screen flex flex-col px-6 md:px-12 lg:px-20 py-12 md:py-20 max-w-[1920px] mx-auto w-full">
            ...
            <div key={colIdx} className="flex flex-col gap-6 md:gap-8 flex-1">
              {col.map((item) => (
                <GalleryCard
                  ...
                  isDevMode={isDevMode}
                  onRemove={(e, tweetId, imageUrl) => handleRemoveLike(tweetId, imageUrl, item.uniqueKey)}
                  onClick={() => {
      ...
      {selectedGridItemIndex !== null && gridItems[selectedGridItemIndex] && (
        <Lightbox
          ...
          isDevMode={isDevMode}
          onRemove={(removedId, removedImageUrl) => {
            handleRemoveLike(removedId, removedImageUrl, gridItems[selectedGridItemIndex].uniqueKey);
          }}
        />
      )}
